...
def run(reltoid, idtoprompt, store_path, args):
    ...
    # 得到一个字典，键：数据集的关系id；值：数据集中属于该关系的句子
    ...
    # 从 flat_examples 中随机抽取 args.num_test 个样本，组成 test_examples 列表，用于测试。
    ...
    if args.use_ft:
        ...
        # 将 example_dict 中所有关系类别的样本展平成一个列表 train_list
        train_list = [x for y in example_dict.values() for x in y]
        # 根据 args.no_na 和 args.task 的设置过滤掉没有实际关系的样本
        if args.no_na:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="ace05", choices=["ace05","semeval","tacred","scierc","wiki80"])
    parser.add_argument("--model", default="text-davinci-003", type=str)
    parser.add_argument("--num_test", type=int, default=2442)
    parser.add_argument("--example_dataset", type=str, default="./dataset/ace05/train.json")
    parser.add_argument("--test_dataset", type=str, default="./dataset/ace05/test.json")
    parser.add_argument("--fixed_example", type=int, default=0)
    parser.add_argument("--fixed_test", type=int,default=1)
    parser.add_argument("--num_per_rel", type=int, default=0)
    ...
    parser.add_argument("--use_knn", type=int, default=1)
    ...
    parser.add_argument("--entity_info", type=int, default=1)
    ...
    if args.no_na and args.num_na != 0:
        ...
        assert False
    # 构建路径
    store_path = "./results/knn_{}_results/test={}_knn={}".format(
        args.task, args.num_test, args.k
    )
